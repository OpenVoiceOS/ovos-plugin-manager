...
class TestAudioBackend(unittest.TestCase):
    ...
    def test_ocp_start_stop_start_without_track_emits_again(self) -> None:
        """start, stop, start with no track loaded must emit PLAYING twice."""
        with patch.object(self.backend.bus, "emit") as emit_mock:
            self.backend.ocp_start()
            self.backend.ocp_stop()
            self.backend.ocp_start()
        playing = [c for c in emit_mock.call_args_list
                   if c.args[0].msg_type == "ovos.common_play.player.state"
                   and c.args[0].data.get("state") == PlayerState.PLAYING]
        self.assertEqual(len(playing), 2)

    def test_ocp_start_error_start_without_track_emits_again(self) -> None:
        """start, error, start with no track loaded must emit PLAYING twice."""
        with patch.object(self.backend.bus, "emit") as emit_mock:
            self.backend.ocp_start()
