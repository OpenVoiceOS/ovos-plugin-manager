...
class TestMediaBackend(unittest.TestCase):
    ...
    def test_ocp_start_stop_start_without_track_plays_again(self) -> None:
        """start, stop, start with no track loaded must call play() twice."""
        with patch.object(self.backend, "play") as play_mock:
            self.backend.ocp_start()
            self.backend.ocp_stop()
            self.backend.ocp_start()
    ...
    def test_ocp_start_error_start_without_track_plays_again(self) -> None:
        """start, error, start with no track loaded must call play() twice."""
        with patch.object(self.backend, "play") as play_mock:
            self.backend.ocp_start()
            self.backend.ocp_error()
            self.backend.ocp_start()
